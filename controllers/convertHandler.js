const { unitPairs, conversionValues, unitDescriptions } = require("../constants/const");
...
function ConvertHandler() {
  ...
  this.spellOutUnit = function (unit) {
    ...
    if (unitPairs.hasOwnProperty(unit.toLowerCase())) {
  ...
  this.convert = function (initNum, initUnit) {
    return Math.round(conversionValues[initUnit] * initNum * 1e5) / 1e5;
  };

  this.getString = function (initNum, initUnit, returnNum, returnUnit) {
    return `${initNum} ${unitDescriptions[initUnit]} converts to ${returnNum} ${unitDescriptions[returnUnit]}`;
  };
}
